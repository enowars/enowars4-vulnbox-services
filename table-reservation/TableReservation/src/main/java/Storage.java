...
public class Storage {
    ...
    public String purify(String str) {
        ...
        for(String term: fList){
            if(tmp.contains(normalize(term)) || tmp.contains(normalize(term).toLowerCase()) || tmp.contains(normalize(term).toUpperCase()) ) {
                return "";
            }
        }
        return str;
    ...
    public LinkedList<String[]> requestReservations(String token) {
        ...
        LinkedList<String[]> meltingPot = new LinkedList<>();
        for(String hex: tmp) {
            if(!hex.toLowerCase().equals("id")){
                String purifiedString = purify(hex);
                if(purifiedString.equals("")) {
                    return new LinkedList<String[]>();
                }
                String sql = "SELECT * FROM reservations WHERE id = " + purifiedString;
                try (ResultSet results = statement.executeQuery(sql);) {
                    ResultSetMetaData meta = results.getMetaData();
                    while (results.next()) {
                        String[] answers = new String[meta.getColumnCount()];
